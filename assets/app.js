...
function toBib(item){
  const type = item.arxiv && (!item.doi || item.type==='accepted') ? 'misc' : 'article';
  ...
  box.innerHTML = list.map(item => `
    <article class="card">
      <div class="card-head">
        ...
        <span class="type">${item.type==='accepted'?'Accepted':'Published'}</span>
      ...
      <div class="links">
        <a href="${item.link}" target="_blank" rel="noopener">期刊页</a>
...
function cardFromItem(item){
  ...
  type.textContent = item.type === 'accepted' ? 'Accepted' : 'Published';
...
async function load(){
  applyLegend(); renderFavCount();
  const resp = await fetch('data/articles.json', {cache:'no-store'});
  const json = await resp.json();
  const itemsRaw = json.items || [];
  const items = itemsRaw.map(normalize).filter(it=>withinDays(it.date));
  ...
  filtered.forEach(it=> box.appendChild(cardFromItem(it)) );
}
